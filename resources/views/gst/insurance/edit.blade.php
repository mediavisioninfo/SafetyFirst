...
@push('script-page')
    ...
    <script>
        $('#policy').on('change', function () {
            "use strict";
            var policy = $(this).val();
            var edit_policy_terms = $('#edit_policy_terms').val();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    pricingList.forEach(function(item) {
                        var checked = edit_policy_terms == item.duration_month ? ' checked' : '';
                        pricing += '<div class="form-check custom-chek form-check-inline"><input class="form-check-input" type="radio" value="' + item.duration_terms + '-' +item.duration_month+'-'+item.price + '" id="' + item.duration_terms + '" name="policy_terms" '+checked+'><label class="form-check-label" for="' + item.duration_terms + '">' + item.duration_terms + ' - ' + priceSymbole+item.price + '</label></div>';
                    });
...
@endpush
@section('content')
    {{Form::model($insurance, array('route' => array('insurance.update', $insurance->id), 'method' => 'PUT')) }}
    <input type="hidden" name="edit_policy_terms" id="edit_policy_terms" value="{{$insurance->policy_term}}">
    <div class="row">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-md-6 col-lg-6">
                            ...
                            {!! Form::select('agent', $agent, null, array('class' => 'form-control  basic-select')) !!}
                        </div>
                        <div class="form-group col-md-6 col-lg-6">
                            {{ Form::label('agent_commission', __('Agent Commission'), array('class' => 'form-label')) }}
                            {{ Form::number('agent_commission', null, array('class' => 'form-control','placeholder'=>__('Enter agent commission'))) }}
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group col-md-6 col-lg-6">
                            ...
                            {!! Form::select('status', $status, null, array('class' => 'form-control  basic-select hidesearch')) !!}
                        </div>
