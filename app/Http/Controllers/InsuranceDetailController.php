...
class InsuranceDetailController extends Controller
{
    ...
    public function update(Request $request, $claimId)
    {
        $validated = $request->validate([
            ...
            'cubic_capacity' => 'nullable|integer',
            'seating_capacity' => 'nullable|integer',
            'insurance_start_date' => 'nullable|date',
            'insurance_expiry_date' => 'nullable|date',
            'additional_towing_charges' => 'nullable|integer',
        ]);

        $insuranceDetail = InsuranceDetail::where('claim_id', $claimId)->first();

        if (!$insuranceDetail) {
            // create new insurance detail record
            $validated['claim_id'] = $claimId;
            InsuranceDetail::create($validated);
        } else {
        ...
        return redirect()->route('claim.show', Crypt::encrypt($claimId))
            ->with('success', __('Insurance details saved successfully.'));
    }
